...
setProgress(0);
  ...
  useEffect(() => {
    axios.get("/story/", { headers: { Authorization: "Bearer " + localStorage.getItem("token") } })
      .then(res => {
              ...
              <>
                <div className='post'>
                  <div className='info'>
                    ...
                    {item?.postedBy._id === state?._id && (<i className="material-icons hover-red" onClick={() => deletePost(item._id)} >delete</i>)}
